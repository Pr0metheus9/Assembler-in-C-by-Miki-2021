...
/*function to do stuff with the line and tables. Works using the second scan
 * algorithm*/
int translate_line2(
    char* line, boolean errorFlag, boolean labelFlag, char label_array[32], int* opcode, int* funct)
{
    ...
    if (*line == '.')
    {
        ...
        /*check if the directive is of the entry type (step 4 of algorithm). if not
         * then return 0 (step 3 of algorithm)*/
        if (type != 2)
        {
            return 0;
        }
        ...
        else
        {
            ...
            if (getlabelsec(line, label_array))
            {
                ...
                {
                    /*if so add the entry type to the current type of the label (step 5 of
                     * algorithm)*/
...
/*function to complete the missing parts in the instruction array (step 6 of
 * algorithm)*/
