...
void insertLabel(char label[],int attribute, int value){
  ...
  /*check if linked list head is null, if so this is the first and only value in the linked list*/
  if(head == NULL){
    head = new_Label;
    last = new_Label;
  }
  ...
  /*check if we already have a formed linked list if so make last item in the list*/
...
int label_exists(char label[]){
  ...
  /*if label name is not already in use return false (because if doesn't already exist)*/
  return 0;
}
