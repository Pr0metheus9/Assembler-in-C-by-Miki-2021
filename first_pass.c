...
keyword directives[] = { { "data" }, { "string" }, { "entry" }, { "extern" } };

/*all the possible registers*/
keyword registers[]
    = { { "r0" }, { "r1" }, { "r2" }, { "r3" }, { "r4" }, { "r5" }, { "r6" }, { "r7" } };

/*function that does the first pass it reads input file line and uses other
...
int translate_line(
    char* line, boolean errorFlag, boolean labelFlag, char label_array[32], int* opcode, int* funct)
{
    /*flag to tell if label was found*/
    labelFlag = False;

    /*check if the line is of comment or whitespace type if so we can just skip
     * it*/
    if (*line == ';')
    {
        return 0;
    ...
    /*check if it is a label if so set the label flag to true (following
     * algorithm) and move to the next part of the line*/
    ...
    /*Check if part could be a directive. if it is a valid directive, save to the
     * word table and continue*/
    if (*line == '.')
    {
    ...
    /*instruction check if part is an instruction. if it is then move to the next
     * part and save to table*/
...
int valid_label(char label_array[32], int i)
{
    ...
    if (!((label_array[0] >= 'a' && label_array[0] <= 'z')
            || (label_array[0] >= 'A' && label_array[0] <= 'Z')))
    {
        return 0;
    }
    ...
    for (i = 0; i < 16; i++)
    {
        if (!strncmp(instructions[i].str, label_array, 4)
            || !strncmp(instructions[i].str, label_array, 3))
        {
...
{
    /*create array to store label characters as we go char by char in line until
     * we find the end of the label (if it exists) max length is 31*/
    int i = 0;

    /*until you find something indicating the end of the label, add char to array
     * and keep going over char*/
    while (*line != ':' && *line != '\0')
    {
        if (i > 31)
        {
    ...
    for (i = 0; i < 15; i++)
    {
        ...
        if (!strncmp(instructions[i].str, line, 3))
        {
            ...
            *opcode = i;
            ...
            /*To handel instructions with same opcode, use different cases. In
             * instructions which do not "share" their opcode with other instructions,
             * index in struct of instructions is enough. (because of the way we
             * defined instructions array*/
...
{
    /*part 6 of algorithm if it is a label insert into symbol table (linked list)
     * as .data type*/
    if (labelFlag)
    {
    ...
    switch (type)
    {
        ...
        case 0:
            ...
            while (line != NULL && *line != '\0' && *line != ' ' && *line != '\t' && *line != '\n')
            {
                ...
                /*check if number is negative and move to the digits so that we can
                 * handle them*/
                if (*line == '-')
                {
                    signFlag = True;
                ...
                /*if number is negative get the two's complement of the positive value
                 * returned*/
                if (signFlag)
                {
                    value = (~value) + 1;
         ...
         * into the label table with the type of extern and with value 0*/
        case 3:
            if (check_label(line, label_array))
            {
                existsExt = True;
...
{
    /*if there was a label before the instruction insert it to the label table
     * with the value of the IC and type of code*/
    int L = -1;
    /*declares and array of 3 possible memory words (max number of words) one for
     * each of command, source and target*/
    Word iwords[3];
    ...
    /*sets the right number of operators to be given in any type of instruction
     * part 15 of algorithm*/
    if (*opcode <= 4)
    {
        L = 2;
    }
    ...
    /*for each of the words, if they exist, insert them into the instruction array
     * and increment IC*/
...
/*function to insert the source and target into word if they are relevant using
 * correct addresing type*/
int addressfunc(Word iwords[], int L, char* line, char label_array[32])
{
    int i;

    /*loops where number of iterations is equal to the amount of words we will
     * need to create*/
    for (i = 1; i <= L; i++)
    {
        ...
        else if (*line == '%')
        {
            ...
            /*make empty space for the label address in future (can't be -1 cause we
             * already have that as default for iwords[1,2]*/
            iwords[i].content = 0;
            iwords[i].are = 'A';
            continue;
        }
        ...
        else if (*line == 'r')
        {
            ...
            /*if it is destination operand set destination addressing type to 3 (bits
               2,3)*/
            else if (L == 2 && i == 2)
            {
        ...
        else if (getlabelsec(line, label_array))
        {
            ...
            /*if it is destination operand set destination addressing type to 1 (bits
               2,3)*/
            else if (L == 2 && i == 2)
            {
